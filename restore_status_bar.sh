#!/bin/bash
# Restore tmux-gruvbox to use Powerline symbols (Nerd Fonts required)
# Run this if you have installed Nerd Fonts and want the fancy icons back!
...
echo "Restoring $THEME_FILE to use Powerline symbols..."

# Replace ASCII with Powerline separators
# > -> 
# < -> 
# | ->  (careful, context matters, but likely safe for this specific theme file structure)
# | -> 

# To be safe and precise, we match the context typically used in the theme file.
# The theme uses specific colors next to the separators which helps unique identification,
# but a simple sed replacement might be tricky if '|' is used elsewhere.
# Let's inspect the original file content structure from memory or re-read it if possible.
# Actually, the patch was:
...
# For pipes, it's ambiguous because both left and right separators were mapped to '|'.
# We can't perfectly restore  vs  from just '|' without context.
# However, usually Left Status uses  (right-pointing soft) and Right Status uses  (left-pointing soft).
# Let's try to be smart or just tell the user to reinstall if they want it perfect.

# Better approach: Extract the original file from the tarball if available? No, script must work standalone.
# Best approach for script: Since we have the original content in our chat history/memory, 
# I can just overwrite the file with the known "good" content for the dark theme.
...
echo "Restore complete. Reload tmux (Prefix + r) to see changes."
